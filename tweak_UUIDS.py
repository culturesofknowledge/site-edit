...
for prefix in prefixes:
	...
	if len(keys_string) > 0 :
		...
		for batch in range( begin, key_count, batch_count ) :
			...
			print( prefix.replace("-", ":"), batch, "of", key_count )
			...
			for batch_key in batch_keys :
				...
				if prefix == "cofk_union_comment-" :
					tweaker.update_comment( batch_key.replace(prefix, ""), updater, anonymous=True )
				elif prefix == "cofk_union_image-":
					tweaker.update_image( batch_key.replace(prefix, ""), updater, anonymous=True )
				elif prefix == "cofk_union_institution-":
					tweaker.update_institution( batch_key.replace(prefix, ""), updater, anonymous=True )
				elif prefix == "cofk_union_location-":
					tweaker.update_location( batch_key.replace(prefix, ""), updater, anonymous=True )
				elif prefix == "cofk_union_manifestation-":
					tweaker.update_manifestation( batch_key.replace(prefix, ""), updater, anonymous=True )
				elif prefix == "cofk_union_person-":
					tweaker.update_person( batch_key.replace(prefix, ""), updater, anonymous=True )
				elif prefix == "cofk_union_resource-":
					tweaker.update_resource( batch_key.replace(prefix, ""), updater, anonymous=True )
				elif prefix == "cofk_union_work-":
					tweaker.update_work( batch_key.replace(prefix, ""), updater, anonymous=True )
