...
if (isset($argv[1])) {
	$Ingestname = $argv[1];
	print "Ingestname = " . $Ingestname;
} else {
	print $argv[0]." - Must supply an ingestname ";
	exit(1);
}
...
//define( 'CONSTANT_DATABASE_HOST', $serverPgSQL );
//define( 'CONSTANT_AEOLUS_HOSTNAME', $serverPgSQL );

define( 'CONSTANT_DATABASE_TYPE', 'live' );
//define( 'PDF_DIR', 'http://cofk2.bodleian.ox.ac.uk/culturesofknowledge/transcripts/schottvegelin/' );
...
#----------------------------------------------------------------------------------
	...
	//$regexObj = new MongoRegex("/$Ingestname/i");
	//$where["title"] = $regexObj;
	//$uploads = $upload->findOne(array('upload_id'=> $Ingestname),array('_id' => 0) );
	...
	print_r( "Uploads:", $uploads);

	#-----------------------------------
	# Record that the upload has started
	#-----------------------------------
	$uploads['upload_id'] = $upload_id ;
//		$uploads['upload_description']  .= " date " || date("D M j G:i:s T Y");  // Sat Mar 10 17:16:18 MST 2001
	$uploads['upload_description']  = $Ingestname. " " . date("j M Y G:i");    // Sat Mar 10 17:16:18 MST 2001
	$uploads['upload_timestamp'] = date('Y-m-d h:i:s', $uploads['upload_timestamp']->sec);
  print_r($uploads);
 //  Test code to this point
//exit;
	pg_insert ( $db_connection->connection->connection,
							'cofk_collect_upload' ,
...
function get_file_to_tables_lookup() {
  ...
  function transfer_temp_table_to_perm() {  # transfer into the 'collect work' table
    ...
    $cols = $cofk->db_list_columns( $cofk->perm_postgres_table );
    $num_saved = 0;

    foreach( $cofk->uploaded_data as $row ) {
      extract( $row, EXTR_OVERWRITE );
      $num_saved++;
      echo "Saving '" . $cofk->table_desc .  "' record no. $num_saved ..." ;
      echo LINEBREAK;

      $cols_statement = "insert into $cofk->perm_postgres_table ( upload_id";
      $vals_statement = "values ( $cofk->upload_id";
      ...
      foreach( $cols as $crow ) {
        ...
        if( $is_numeric )
          ...
          $vals_statement .= "'" . $cofk->escape( $$column_name ) . "'";
      }
      $cols_statement .= ') ';
      $vals_statement .= ') ';
    ...
    echo "Saved $num_saved '" . $cofk->table_desc . "' records." . LINEBREAK;
    echo LINEBREAK;
  }
  #-----------------------------------------------------
#-----------------------------------------------------
