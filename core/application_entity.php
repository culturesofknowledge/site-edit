...
define( 'CMS_POPUP_WINDOW_WIDTH', 800 ); # CMS layout currently (June 09) looks a bit of a mess unless window is wider
define( 'APP_POPUP_SELECTFORM_NAME', 'app_popup_selectform' );

define( 'CMS_DB_USERNAME', 'webuser' );  # A user with read-only role when doing publicly available search via CMS
...
define( 'HELP_DB_USERNAME', 'webuser' );  # A user with read-only permissions on help pages (without login)

class Application_Entity {
  ...
  function clear() { # Clear all properties except DB connection, debug and magic quotes setting
    ...
    $keep_magic_quotes_gpc_on = $this->magic_quotes_gpc_on;
    ...
    $this->magic_quotes_gpc_on = $keep_magic_quotes_gpc_on;
  ...
  function search_startup( $class_name ) {  # Goes straight into DB search. No logging in required.
    ...
    else {  # We will only allow methods ending with '_search_results', to try and hamper the efforts of hackers.
  ...
  function is_search_results_method( $method_name ) {
  ...
  function get_supervisor() {    # N.B. We must create a username of pattern "cfg prefix + super" for each DB
                                 # otherwise password change option will not work.
    return $this->get_system_prefix() . 'super';
  }
  #-----------------------------------------------------
