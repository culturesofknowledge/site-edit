...
csv_file = "resources/bayle_manifestations/TIDYING_Bayle_to_upload_2016.9.28.csv"
csv_rows = tweaker.get_csv_data( csv_file )

for csv_row in csv_rows:
	print( ".", end="" )

	manifestation = tweaker.get_manifestation_from_manifestation_id( csv_row["manifestation_id"])

	update = {
		'printed_edition_details': csv_row['printed_edition_details'],
		'id_number_or_shelfmark': csv_row['id_number_or_shelfmark']
	}
	...
	if csv_row['notes'] != '' :
		...
		if len(relationships) == 0 :
			print( "MISSING A NOTE", csv_row["iwork_id"], csv_row["manifestation_id"] )

		elif len(relationships) > 1 :
			print(  csv_row["iwork_id"], csv_row["manifestation_id"], "Too many" )
